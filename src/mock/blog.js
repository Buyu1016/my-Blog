...
Mock.mock(/^\/api\/blog(\?.+)?$/, "get", function(options) {
        const limit = queryString.parse(options.url).limit;
        return Mock.mock({
            code: 0,
            msg: "",
            "data": {
                ...
                [`rows|${limit||10}`]: [{
                    ...
                    "commentNumber|1-20": 0,
                    "thumb|1": [Mock.Random.image("300x250", "#abcdef", "#fcc", "@title"), null],
...
Mock.mock(/^\/api\/blog\/[^/]+$/, "get", {
    ...
    data: {
        ...
        "scanNumber|10-2000": 0, //浏览次数
        "commentNumber|1-100": 0, //评论数
        description: "@cparagraph(1, 10)",
        createDate: "@DATETIME('yyy-MM-dd')", //时间戳，创建日期
        toc: [
...
Mock.mock("/api/comment", "post", {
    ...
    data: {
        ...
        createDate: "@DATETIME('yyy-MM-dd')",
        avatar: Mock.Random.image("100x100", "#abcdef", "#fcc", "CodeGorgeous")
    }
});
/**
 * 分页博客数据
...
Mock.mock(/\/api\/comment\/?(\?.+)?$/, "get", function(options) {
    const query = queryString.parse(options.url);
    return Mock.mock({
        code: 0,
        msg: "",
        data: {
